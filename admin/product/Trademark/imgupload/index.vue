...
const confirm = async () => {
    // 在发请求之前，对整个表单进行校验
    // 调用这个方法进行全部表单检验，只有校验通过，才执行后面的操作
...
const validatorTmName = (rule:any,value:any,callBack:any) => {
    ...
    if(value.trim().length >= 2) {
        ...
        // 校验未通过返回的错误提示信息
        callBack(new Error('品牌名称小于两位'))
    }
}

// 品牌logo的自定义检验规则方法
const validatorLogUrl = (rule:any,value:any,callBack:any) => {
    if(value) {
        callBack();
    } else {
...
const rules = {
    ...
    logUrl: {required: true, validator:validatorLogUrl}
}
</script>
